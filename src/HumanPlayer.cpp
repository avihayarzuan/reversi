...
void HumanPlayer::connectToServer() {
    // Create a socket point
        ...
        if (connect(clientSocket, (const struct sockaddr *)&serverAddress, sizeof(serverAddress)) == -1) {
...
int HumanPlayer::readMessage(char *p) {
    int stat;
//    char returnBuff[256];
    stat = read(clientSocket, p, sizeof(char) * 256);
    if (stat == -1) {
